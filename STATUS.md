...
- A **plumbing layer**: it polls Miniflux, verifies and reacts to Miniflux's native webhook, and exposes that as HA-native primitives —
  ...
  - **2 Lovelace dashboard cards**, auto-delivered (no manual resource step on storage-mode dashboards): `custom:miniflux-feed-manager-card` and `custom:miniflux-category-manager-card` — full feed/category CRUD management surfaces, no entry content rendered. See `plans/cards/detailed/PHASE-1-PROGRESS.md` for the frontend's own status; this file stays backend-focused.
...
- **Not yet run inside a real Home Assistant instance.** Every test — all 469 of them — runs against `pytest-homeassistant-custom-component`'s simulated HA test harness (and the frontend's 241 against Vitest/happy-dom). Both harnesses are high-fidelity, but still simulations. This integration has never actually loaded inside a genuine running Home Assistant, and the two dashboard cards have never been added to a real dashboard. The `F-U1` delivery spike was validated in a real HA earlier; the full card set built on top of it since has not yet had its own real-HA pass.
